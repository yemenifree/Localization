...
class Negotiator implements NegotiatorContract
{
    ...
    {
        if ( ! is_null($locale = $this->getFromAcceptedLanguagesHeader($request)))
            return $locale;

        if ( ! is_null($locale = $this->getFromHttpAcceptedLanguagesServer($request)))
            return $locale;

        if ( ! is_null($locale = $this->getFromRemoteHostServer($request)))
            return $locale;
     ...
     * @return string|null
     */
    protected function getFromAcceptedLanguagesHeader(Request $request)
    {
     ...
     * @return string|null
     */
    protected function getFromHttpAcceptedLanguagesServer(Request $request)
    {
        $httpAcceptLanguage = $request->server('HTTP_ACCEPT_LANGUAGE');
     ...
     * @return string|null
     */
    protected function getFromRemoteHostServer(Request $request)
    {
        if (empty($remoteHost = $request->server('REMOTE_HOST')))
            return null;
